...
def init_db():
    ...
    user2 = User('example2@email.com', 'password', 'Valjean', 'Jean', datetime.strptime('1977-03-18', '%Y-%m-%d'), '0322334459', 50)
    user3 = User('example3@email.com', 'pass', 'Gasaï', 'Yuno', datetime.strptime('1999-11-22', '%Y-%m-%d'), '0999999999', 999)
    user4 = User('example4@email.com', 'passu', 'Marie', 'Julien', datetime.strptime('1999-11-14', '%Y-%m-%d'), '0999777999', 99)
    user5 = User('example5@email.com', 'azerty', 'Duminy', 'Gaétan', datetime.strptime('1995-06-18', '%Y-%m-%d'), '0601020304', 150)
    ...
    db.session.commit()
    # Ads
    ad1 = Ad(datetime.strptime('2018-01-13', '%Y-%m-%d'), datetime.strptime('2018-01-14', '%Y-%m-%d'), '15x15x15', 50, 0, '3 rue liberté 03333 BELETTES', '3 rue de l\'élite 09333 SAINT-GRACIEN', user1.id)
    ad2 = Ad(datetime.strptime('2018-01-22', '%Y-%m-%d'), datetime.strptime('2018-01-23', '%Y-%m-%d'), '15x15x15', 50, 0, '53 avenue de laplaine 01111 MEULIN', '57 rue sintillante 01111 MEULIN', user4.id)
    ad3 = Ad(datetime.strptime('2018-01-24', '%Y-%m-%d'), datetime.strptime('2018-01-25', '%Y-%m-%d'), '15x15x22', 50, 0, '53 avenue de laplaine 01111 MEULIN', '57 rue sintillante 01111 MEULIN', user4.id)
    ...
    db.session.add(ad4)
    db.session.add(ad5)
    db.session.add(Ad(datetime.strptime('2018-01-18', '%Y-%m-%d'), datetime.strptime('2018-01-19', '%Y-%m-%d'), '15x15x30', 50, 0, '3 rue ferry 03333 ST-LAURENT', '3 rue crocus 09333 SAINT-LAURENT', user2.id))
    db.session.add(Ad(datetime.strptime('2018-01-22', '%Y-%m-%d'), datetime.strptime('2018-01-24', '%Y-%m-%d'), '15x15x74', 45, 0, '3 rue bonaparte 03333 VILLENEUVE', '3 rue ibiscus 09333 SAINT-LAURENT', user1.id))
    db.session.commit()
    # Offers
    offer1 = Offer(datetime.strptime('2018-01-13', '%Y-%m-%d'), '10x10x10', 33, '1 avenue du printemps 01000 AIX', '999 route de l\'hiver 99999 YVES', user2.id, ad1.id)
    offer2 = Offer(datetime.strptime('2018-01-02', '%Y-%m-%d'), '10x10x10', 33, '1 avenue du printemps 01000 AIX', '999 route de l\'hiver 99999 YVES', user4.id, ad2.id)
    offer3 = Offer(datetime.strptime('2018-01-06', '%Y-%m-%d'), '10x10x10', 33, '1 avenue du printemps 01000 AIX', '999 route de l\'hiver 99999 YVES', user1.id, ad3.id)
